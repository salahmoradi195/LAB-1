...
ISR(INT1_vect)
{
    ...
    if (rising)
    {
        ...
        TCNT1 = 0;
        rising = 0;
        EICRA &= ~(1 << ISC10);     // switch to falling edge
        EICRA |=  (1 << ISC11);
    }
    else
    {
        ...
        echo_done = 1;
        rising = 1;
        EICRA |=  (1 << ISC10) | (1 << ISC11);  // switch back to rising
    }
}

void timer1_init()
{
    // Normal mode, no prescaler → 16 MHz counting
    TCCR1A = 0x00;
    TCCR1B = (1 << CS10);
}
...
void pwm_init()
{
    // Fast PWM on OC0A (PD6)
    TCCR0A = (1 << COM0A1) | (1 << WGM00) | (1 << WGM01);
    TCCR0B = (1 << CS01); // prescaler 8 → ~7.8 kHz PWM
...
uint16_t measure_distance_cm()
{
    ...
    // distance (cm) = echo_time / 932 (for 16 MHz clock)
